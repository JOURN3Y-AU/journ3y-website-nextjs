...
const industries: IndustryData[] = [
  ...
  },
  {
    ...
    relatedIndustries: ["professional-services", "real-estate", "retail"]
  },
  {
    slug: "hospitality",
  ...
  },
  {
    ...
    relatedIndustries: ["construction", "retail", "professional-services"]
  }
];
...
async function seed() {
  ...
  for (const industry of industries) {
    ...
    if (indError) {
      console.error(`  ❌ Error inserting ${industry.slug}:`, indError.message);
      continue;
    }
    ...
    for (let i = 0; i < industry.painPoints.length; i++) {
      const { error } = await supabase.from('smb_pain_points').insert({
        industry_id: ind.id,
        title: industry.painPoints[i].title,
        description: industry.painPoints[i].description,
        display_order: i,
      });
      if (error) console.error(`  ⚠️ Pain point error:`, error.message);
    }

    // Insert use cases
    for (let i = 0; i < industry.useCases.length; i++) {
      const { error } = await supabase.from('smb_use_cases').insert({
        industry_id: ind.id,
      ...
      if (error) console.error(`  ⚠️ Use case error:`, error.message);
    }

    // Insert FAQs
    for (let i = 0; i < industry.faqs.length; i++) {
      const { error } = await supabase.from('smb_faqs').insert({
        industry_id: ind.id,
        question: industry.faqs[i].question,
        answer: industry.faqs[i].answer,
        display_order: i,
      });
      if (error) console.error(`  ⚠️ FAQ error:`, error.message);
    }

    console.log(`  ✅ Seeded ${industry.slug} with ${industry.painPoints.length} pain points, ${industry.useCases.length} use cases, ${industry.faqs.length} FAQs`);
  }

  console.log('\n🎉 Done seeding SMB industries!');
}
